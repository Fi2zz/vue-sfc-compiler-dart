...
JSFunctionExpression _buildClientSlotFn(Object? props, Object? vForExp,
    List<TmplNode> children, TmplLoc loc) {
  return JSFunctionExpression(props, children,
      newline: false,
      isSlot: true,
...
SlotsBuildResult buildSlots(ElementNode node, TransformContext context) {
  ...
  if (!context.ssr && context.prefixIdentifiers) {
    st.hasDynamicSlots = node.props.any((prop) =>
            isVSlot(prop) &&
...
void _buildOnComponentSlot(DirectiveNode onComponentSlot, ElementNode node,
    TransformContext context, _BuildSlotsState st, TmplLoc loc) {
  final arg = onComponentSlot.arg;
  final exp = onComponentSlot.exp;
  if (arg != null && !isStaticExp(arg)) {
    st.hasDynamicSlots = true;
  }
  st.slotsProperties.add(createObjectProp(
      arg ?? createSimpleExp('default', true),
      _buildClientSlotFn(exp, null, node.children, loc)));
}

void _buildTemplateSlots(ElementNode node, TransformContext context,
    _BuildSlotsState st, bool hasOnComponentSlot) {
  final children = node.children;
  for (var i = 0; i < children.length; i++) {
    ...
    if (hasOnComponentSlot) {
      context.onError(TmplCompileError(
          37, 'Slot usage mixed on component and template.', slotDir!.loc));
      break;
    }
    st.hasTemplateSlots = true;
    _buildOneTemplateSlot(
        slotElement as ElementNode, slotDir!, context, st, i, children);
  }
}

void _buildOneTemplateSlot(ElementNode slotElement, DirectiveNode slotDir,
    TransformContext context, _BuildSlotsState st, int index,
  ...
  final slotFunction = _buildClientSlotFn(slotProps, vFor, slotChildren, slotLoc);
...
void _slotWithIf(DirectiveNode vIf, Object slotName,
    JSFunctionExpression slotFunction, _BuildSlotsState st) {
  st.hasDynamicSlots = true;
  st.dynamicSlots.add(JSConditionalExpression(
      vIf.exp!,
      _buildDynamicSlot(slotName, slotFunction, st.conditionalBranchIndex++),
      _defaultFallback));
}

void _slotWithElse(DirectiveNode vElse, Object slotName,
    JSFunctionExpression slotFunction, _BuildSlotsState st,
    TransformContext context, List<TmplNode> children, int index) {
  var j = index;
  TmplNode? prev;
  while (j-- > 0) {
...
void _attachElseSlot(DirectiveNode vElse, Object slotName,
    JSFunctionExpression slotFunction, _BuildSlotsState st) {
  ...
  conditional.alternate = vElse.exp != null
      ? JSConditionalExpression(
          vElse.exp!,
          _buildDynamicSlot(
              slotName, slotFunction, st.conditionalBranchIndex++),
          _defaultFallback)
      : _buildDynamicSlot(
          slotName, slotFunction, st.conditionalBranchIndex++);
}

void _slotWithFor(DirectiveNode vFor, Object slotName,
    JSFunctionExpression slotFunction, _BuildSlotsState st,
  ...
  if (parseResult != null) {
    ...
    st.dynamicSlots.add(createCallExp(hRenderList, [
      parseResult.source,
      JSFunctionExpression(createForLoopParams(parseResult),
...
void _plainSlot(Object slotName, String? staticSlotName,
    JSFunctionExpression slotFunction, _BuildSlotsState st,
    TransformContext context, DirectiveNode slotDir) {
...
void _buildDefaultSlot(ElementNode node, TransformContext context,
    _BuildSlotsState st, TmplLoc loc) {
  JSProperty buildDefaultSlotProperty(
      Object? props, List<TmplNode> children2) {
  ...
  if (!st.hasTemplateSlots) {
    ...
    if (st.hasNamedDefaultSlot) {
      ...
      st.slotsProperties
          .add(buildDefaultSlotProperty(null, st.implicitDefaultChildren));
    }
  }
}

SlotsBuildResult _assembleSlots(_BuildSlotsState st, TransformContext context,
    TmplLoc loc, ElementNode node) {
