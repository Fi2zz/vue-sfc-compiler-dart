...
final transformFor =
  ...
  return _processFor(node, dir, context, (forNode) {
    ...
    if (memo != null && keyExp != null && isDirKey) {
      keyProp.exp =
          keyExp = processExpression(keyExp as SimpleExpression, context);
    }
    final keyProperty =
    ...
    final isStableFragment =
        src is SimpleExpression && src.constType > 0;
    final fragmentFlag =
        isStableFragment ? 64 : (keyProp != null ? 128 : 256);
    ...
    forNode.codegenNode = createVNodeCall(
        ...
        VNodeCallSpec(hFragment,
            ...
            loc: node.loc));
    return () => _finishForCodegen(node, dir, context, forNode, renderExp,
        isTemplate, memo, keyProp, keyProperty, isStableFragment);
  });
});
...
void _processTemplateMemoAndKey(TmplNode node, DirectiveNode? memo,
    Object? keyProp, JSProperty? keyProperty, TransformContext context) {
...
void _finishForCodegen(
  ...
  if (slotOutlet != null) {
    ...
    childBlock = createVNodeCall(
        ...
        VNodeCallSpec(hFragment,
            props:
                keyProperty != null ? createObjectExp([keyProperty]) : null,
            children: (node as ElementNode).children,
            patchFlag: 64,
            isBlock: true));
  } else {
    childBlock = _adjustChildBlock(children[0] as ElementNode, isTemplate,
        keyProperty, isStableFragment, context);
  }
  _pushForRenderArgs(
      renderExp, forNode, memo, keyProperty, childBlock, context);
}
...
Object _adjustChildBlock(ElementNode child, bool isTemplate,
    JSProperty? keyProperty, bool isStableFragment, TransformContext context) {
...
void _pushForRenderArgs(JSCallExpression renderExp, ForNode forNode,
    DirectiveNode? memo, JSProperty? keyProperty, Object childBlock,
    TransformContext context) {
  if (memo != null) {
    _pushMemoLoop(renderExp, forNode, memo, keyProperty, childBlock, context);
  } else {
    renderExp.arguments.add(JSFunctionExpression(
        createForLoopParams(forNode.parseResult), childBlock,
        newline: true));
  }
}

void _pushMemoLoop(JSCallExpression renderExp, ForNode forNode,
    DirectiveNode memo, JSProperty? keyProperty, Object childBlock,
    TransformContext context) {
  final loop = JSFunctionExpression(
      createForLoopParams(forNode.parseResult,
          [createSimpleExp('_cached')]),
      null);
...
Object? _processFor(TmplNode node, DirectiveNode dir,
    TransformContext context, Object? Function(ForNode forNode) processCodegen) {
  if (dir.exp == null) {
    context.onError(
  ...
  final forNode = ForNode(parseResult.source, parseResult,
      isTemplateNode(node) ? (node as ElementNode).children : [node], dir.loc,
...
void _forIdentifiers(
    TransformContext context, ForParseResult result, bool add) {
  if (!context.prefixIdentifiers) return;
  void Function(Object?) op =
      add ? context.addIdentifiers : context.removeIdentifiers;
...
void finalizeForParseResult(ForParseResult result, TransformContext context) {
  ...
  if (context.prefixIdentifiers) {
    result.source =
        processExpression(result.source as SimpleExpression, context);
    if (result.key != null) {
      result.key = processExpression(result.key! as SimpleExpression, context,
          asParams: true);
    }
    if (result.index != null) {
      result.index = processExpression(
          result.index! as SimpleExpression, context,
          asParams: true);
    }
    if (result.value != null) {
      result.value = processExpression(
          result.value! as SimpleExpression, context,
          asParams: true);
    }
  }
...
List<Object?> createForLoopParams(ForParseResult result,
    [List<SimpleExpression> memoArgs = const []]) {
  return _createParamsList(
      [result.value, result.key, result.index, ...memoArgs]);
}
